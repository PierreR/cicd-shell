...
run (Options zone (Stack (stack, StackData (key, Just role)))) = runCommand zone "stack_data_for_on" (key : role: toList stack)
run (Options zone (Stack (stack, StackFacts Nothing)))         = runCommand zone "stack_facts"  (toList stack)
run (Options zone (Stack (stack, StackFacts (Just role))))     = runCommand zone "stack_facts_on" (role: toList stack)
...
run (Options zone (Stack (stack, StackPing (Just role))))      = runCommand zone "stack_ping_on" (role: toList stack)
run (Options zone (Stack (stack, StackRunPuppet role )))       = runCommand zone "stack_runpuppet_on" (role : toList stack)
run (Options zone (Stack (stack, StackSync Nothing)))          = runCommand zone "stack_sync" (toList stack)
run (Options zone (Stack (stack, StackSync (Just role))))      = runCommand zone "stack_sync_on" (role : toList stack)
