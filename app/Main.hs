...
run (Options zone (Stack (StackData stack key)))            = runCommand zone "stack_data_for" (key : toList stack)
run (Options zone (Stack (StackFacts stack Nothing )))      = runCommand zone "stack_facts"  (toList stack)
run (Options zone (Stack (StackFacts stack (Just target)))) = runCommand zone "stack_facts_on" (target: toList stack)
run (Options zone (Stack (StackOrchestrate stack cmd)))     = runCommand zone "stack_orch" (cmd : toList stack)
run (Options zone (Stack (StackPing stack)))                = runCommand zone "stack_ping" (toList stack)
run (Options zone (Stack (StackRunPuppet stack target )))   = runCommand zone "stack_runpuppet_on" (target : toList stack)
run (Options zone (Stack (StackSync stack)))                = runCommand zone "stack_sync" (toList stack)
