...
....
→ cicd
CICD - command line utility (v2.7.0)

Usage: cicd (pass | doc | ZONE (console | facts | runpuppet | setfacts | foreman
            | orch | run | service | state | du | ping | sync | stats | result |
            gentags | validate))
...
Available commands:
  ...
  run                      Execute a command on the master
...
{
  "fqdn": "svappcavlxxx.prd.srv.cirb.lan",
  "ip": "192.xxx.xx.x",
  "os": "RedHat 7.3",
  "hostgroup": "ci",
  "subgroup": "jenkins",
...
{
  "svappcavlxxx.prd.srv.cirb.lan": true
}
```
...
```
→ cicd staging du -n svappcavlxxx.sta.srv.cirb.lan
```
...
Execute a command (called 'runner' in Salt) on the salt master.

```
→ cicd prod run CMD -n NODE
```

This command would usually concern a single node:

.clean certificats (will only work in production zone)
```
→ cicd prod run puppet.clean_certs -n NODE
...
[%autofit]
```
$ pep -G 'hostgroup:iam' file.replace '/etc/resolv.conf' pattern='192.168.xx.x' repl='192.168.xx.x' <1>
...
```
nix-env -i -f https://github.com/CIRB/cicd-shell/tarball/v2.6.2
```
