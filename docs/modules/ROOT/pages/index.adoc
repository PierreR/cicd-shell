...
./docker/run.sh dev facts <2>
```
<1> login to the Artifactory docker registry is mandatory even for pulling. Use your encrypted password.
<2> look at http://stash.cirb.lan/projects/CICD/repos/cicd-shell/browse/docker/run.sh[docker/run.sh] script and adjust it to your setting
...
```
nix-env -f ~/.config/nixpkgs/pin.nix -iA cicd-shell <1>
```
